...
AI が真に解決不能と判断した場合のみ `## Status: failed` を Result に書く。`process-runner.sh` は `grep` で検出して `done/` に移さず todo に残す。再試行時は AI に「前回失敗の記録」として読ませる。
...
`plan` の対話 claude を抜けた後、`REVIEW_SPEC` で指定された md ファイル（既定: スクリプトと同じディレクトリの `process-runner-review.md`）を読み込み、その `## H2` セクション数ぶんの **read-only レビュータスク** を todo に自動追加する。
