...
# Usage:
#   cd ~/myproject
#   process-runner.sh plan [name]   — 対話claudeで壁打ち。DESIGN.md とタスクを合意
#   process-runner.sh add  "<task>" — タスクファイルを追加
#   process-runner.sh next          — ready な先頭1件を実行
#   process-runner.sh run-all       — ready なものを並列実行（JOBS=並列度, 既定20）
#   process-runner.sh show          — 状況表示（DESIGN/TODO/DONE/LOGS）
#   process-runner.sh reset         — done→todo 戻す + Result削除 + ログ削除（DESIGN.md 維持）
#
# plan は対話 claude を抜けた後、実装タスクが 1 つ以上 todo にあれば
# REVIEW_SPEC の md ファイル (既定: スクリプトと同じディレクトリの
# process-runner-review.md) を読んで、その H2 セクションごとに 1 本の
...
EFFORT="${EFFORT:-}"
STREAM="${STREAM:-1}"
REVIEW_SPEC="${REVIEW_SPEC:-$(cd "$(dirname "$0")" && pwd)/process-runner-review.md}"
...
環境変数:
  ...
  REVIEW_SPEC=PATH plan 後に読むレビュー観点ファイル（既定: スクリプトと同じディレクトリの process-runner-review.md）
