...
AI が真に解決不能と判断した場合のみ `## Status: failed` を Result に書く。`claude-runner.sh` は `grep` で検出して `done/` に移さず todo に残す。再試行時は AI に「前回失敗の記録」として読ませる。
...
`plan` の対話 claude を抜けた後、`REVIEW_SPEC` で指定された md ファイル（既定: スクリプトと同じディレクトリの `claude-runner-review.md`）を読み込み、その `## H2` セクション数ぶんの **read-only レビュータスク** を todo に自動追加する。
...
plan の対話終了
  ...
  generate_review_tasks
        ...
        - depends: 実装タスク全部の番号
        - 本文: 観点ファイルの「## XXX セクションを読め」 + 出力先 (~/Desktop/<workdir-slug>/review-<slug>.md)
                │
                ▼
...
- 出力先を `~/Desktop` 配下のフォルダにまとめる（workdir は `/tmp` で消えるため）
